                ...
                {userData.requiredGames.map((slug) => {
                  ...
                  return (
                    <Link
                      key={slug}
                      href={`/play/${slug}`}
                      className={`relative flex flex-col items-center p-2 rounded-lg border-2 border-black transition-all hover:scale-105 cursor-pointer ${isPlayed ? "bg-emerald-600/50 border-emerald-400" : "bg-slate-800 border-slate-600 hover:border-cyan-400"
                        }`}
                      title={game.name}
                    >
                      <div className="text-2xl mb-1">{game.icon}</div>
                      {isPlayed && (
                        <div className="absolute -top-2 -right-2 bg-emerald-500 rounded-full w-5 h-5 flex items-center justify-center border border-black shadow-sm">
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5, 6, 7].map((day) => {
              ...
              return (
                <div
                  key={day}
                  className={`h-14 w-14 rounded-xl border-[3px] border-black flex items-center justify-center font-bold text-lg ${isActive
