...
/**
 * Server functions to fetch top scores for Enigma Scroll
 */
...
export interface EnigmaScrollLeaderboardData {
...
export async function getTopGlobalScores(): Promise<TopScoresByDifficulty> {
  const leaderboards = await getEnigmaScrollLeaderboards(3);
...
export async function getEnigmaScrollLeaderboards(
  limit = 10
): Promise<EnigmaScrollLeaderboardData> {
