...
/**
 * Enigma Scroll Leaderboard Component
 * 
 * Displays three separate leaderboards (easy=4 letters, medium=5 letters, hard=6 letters)
 * by filtering the game_scores table on the difficulty column.
 * 
...
import {
  getEnigmaScrollLeaderboards,
  type EnigmaScrollLeaderboardData,
} from "./get-top-scores";
...
interface EnigmaScrollLeaderboardProps {
  initialDifficulty?: Difficulty;
}

type LeaderboardData = EnigmaScrollLeaderboardData;

export function EnigmaScrollLeaderboard({ initialDifficulty = "easy" }: EnigmaScrollLeaderboardProps) {
  ...
  useEffect(() => {
    ...
    async function fetchLeaderboards() {
      ...
      try {
        const leaderboardData = await getEnigmaScrollLeaderboards();

        if (!cancelled) {
          setLeaderboards(leaderboardData);
        }
      } catch (error) {
        console.error("Error fetching Enigma Scroll leaderboards:", error);
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
