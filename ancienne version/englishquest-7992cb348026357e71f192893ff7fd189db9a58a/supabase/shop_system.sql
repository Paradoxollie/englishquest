...
create table if not exists public.shop_items (
  ...
  -- Type d'item: 'avatar', 'title', 'background', 'ship_skin'
  item_type text not null check (item_type in ('avatar', 'title', 'background', 'ship_skin')),
...
create table if not exists public.user_equipped_items (
  ...
  -- Item équipé pour le skin de vaisseau
  equipped_ship_skin_id uuid references public.shop_items(id) on delete set null,
  updated_at timestamptz not null default timezone('utc', now())
);
...
-- Admins can manage shop items (insert, update, delete)
...
values
  ...
  -- Ship Skins (skins de vaisseau pour Space Lex Shooter)
