...
export function CustomizationDisplay({ userId, username }: CustomizationDisplayProps) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const { data: equipped, error: equippedError } = await supabase
        ...
        .select(`
          *,
          ...
          equipped_ship_skin:shop_items!equipped_ship_skin_id(*)
        `)
        .eq("user_id", userId)
        .maybeSingle();
      
      if (equippedError) {
        console.error("❌ Error loading equipped items:", equippedError);
      } else {
        console.log("✅ Equipped items loaded:", equipped ? "Yes" : "No");
      }
      ...
      if (ownedError) {
        console.error("❌ Error loading owned items:", ownedError);
      } else {
        console.log("✅ Owned items loaded:", owned?.length || 0, "items");
      }
      ...
      if (freeItemsError) {
        console.error("❌ Error loading free items:", freeItemsError);
      }

      console.log("CustomizationDisplay - Loaded data:", {
        equipped: equipped,
        equippedError: equippedError?.message,
        ownedError: ownedError?.message,
        freeItemsError: freeItemsError?.message,
      ...
      if (equipped) {
        ...
        const equippedData: any = {
          ...
          equipped_background: Array.isArray(equipped.equipped_background) 
            ? equipped.equipped_background[0] 
            : equipped.equipped_background,
          equipped_ship_skin: Array.isArray(equipped.equipped_ship_skin) 
            ? equipped.equipped_ship_skin[0] 
            : equipped.equipped_ship_skin,
        };
        setEquippedItems(equippedData);
      }
