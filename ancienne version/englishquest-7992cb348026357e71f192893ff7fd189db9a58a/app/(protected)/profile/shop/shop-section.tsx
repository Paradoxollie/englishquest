...
export function ShopSection({ userLevel, userGold, userId }: ShopSectionProps) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Load shop items (only active items visible to players)
      ...
      if (itemsError) {
        ...
        console.log("✅ Shop items loaded:", items?.length || 0, "items");
      }
      ...
      if (ownedError) {
        console.error("Error loading user items:", ownedError);
      }
      ...
      console.log("Shop data loaded:", {
        ...
        equippedError: equipped?.error?.message,
      });
      
      // Log detailed error if items are empty
      if (!items || items.length === 0) {
        console.error("⚠️ NO SHOP ITEMS FOUND! This is likely an RLS policy issue.");
